...
compute_estimates <- function(df, Y_name, A_name, Z_list, k_fold_assign_and_CATE,
  ...
  k_fold_output_aiptw_1 <- list()
  k_fold_output_aiptw_0 <- list()
  ...
  for(k in 1:k_folds){
    ...
    # (4) original kth fold data with corresponding treatment decisions
    compute_est_output <- compute_estimate_k(df_est, Y_name, A_name, Z_list,
                                     CATE_model, nuisance_model,
                                     threshold, ps_trunc_level)

    k_fold_output_aiptw_1 <- c(k_fold_output_aiptw_1, list(compute_est_output[[1]]))
    k_fold_output_aiptw_0 <- c(k_fold_output_aiptw_0, list(compute_est_output[[2]]))
    k_fold_treatment_effect <- c(k_fold_treatment_effect, list(compute_est_output[[3]]))

    kth_decision_df <- compute_est_output[[4]]
  ...
  k_fold_output_aiptw_1 <- do.call(rbind, k_fold_output_aiptw_1)
  k_fold_output_aiptw_0 <- do.call(rbind, k_fold_output_aiptw_0)
  ...
  # k_non_na will be the same for aiptw_1 and aiptw_0, find once
  k_non_na <- which(!is.na(k_fold_output_aiptw_1$var_aug))
  k_folds_non_na <- length(k_non_na)
  num_obs <- nrow(k_fold_assign_and_CATE[k_fold_assign_and_CATE$k %in% k_non_na,])

  results <- data.frame(aiptw_1 = mean(k_fold_output_aiptw_1$aiptw, na.rm=TRUE),
                        se_aiptw_1 = sqrt((sum(k_fold_output_aiptw_1$var_aug, na.rm=TRUE) / k_folds_non_na) / num_obs),
                        plug_in_est_1 = mean(k_fold_output_aiptw_1$plug_in_est, na.rm=TRUE),
                        mean_aug_1 = mean(k_fold_output_aiptw_1$mean_aug, na.rm=TRUE),
                        aiptw_0 = mean(k_fold_output_aiptw_0$aiptw, na.rm=TRUE),
                        se_aiptw_0 = sqrt((sum(k_fold_output_aiptw_0$var_aug, na.rm=TRUE) / k_folds_non_na) / num_obs),
                        plug_in_est_0 = mean(k_fold_output_aiptw_0$plug_in_est, na.rm=TRUE),
                        mean_aug_0 = mean(k_fold_output_aiptw_0$mean_aug, na.rm=TRUE),
  ...
  # results from each fold for aiptw_1,
  # results from each fold for aiptw_0,
  # results from each fold for treatment effect,
  # dataset with pid/predictions
  return(list(overall_results = results,
              EY_A1_d1 = k_fold_output_aiptw_1,
              EY_A0_d1 = k_fold_output_aiptw_0,
...
compute_estimate_k <- function(df, Y_name, A_name, Z_list,
  ...
  # d_pred <- ifelse(CATE_pred < -abs(threshold), 1, 0) #NOTE - now this works for ABCD data but not sim data, TODO fix
  if(threshold < 0){
  ...
  calc_aiptw <- function(a, mean_dZ, outcome_model, treatment_model, missingness_model){
    ...
    ### Step 8: compute aiptw estimate = plug-in estimate from 6 + stats::mean(augmentation term from step 7)
  ...
  # E[Y(d) | d(Z) = 1]
  aiptw_1 <- calc_aiptw(1, mean_dZ, outcome_model, treatment_model, missingness_model)

  # E[Y(0) | d(Z) = 1 ]
  aiptw_0 <- calc_aiptw(0, mean_dZ, outcome_model, treatment_model, missingness_model)
  ...
  # E[Y(d) - Y(0) | d(Z) = 1] = E[Y(d) | d(Z) = 1]*P(d(Z) = 1) - E[Y(0) | d(Z) = 1 ]*P(d(Z) = 1)
  treatment_effect <- (aiptw_1[['aiptw']] - aiptw_0[['aiptw']]) * mean_dZ

  # "n" x 3 matrix
  inf_fn_matrix <- cbind(
    aiptw_1$augmentation,
    aiptw_0$augmentation,
  ...
  gradient_g <- matrix(c(
    mean_dZ, -mean_dZ, aiptw_1[['aiptw']] - aiptw_0[['aiptw']]
  ) , ncol = 1)
  ...
  #         (3) overall treatment effect among optimally treated
  #         (4) original kth fold data with corresponding treatment decisions
  return(list(data.frame(aiptw = aiptw_1[['aiptw']], plug_in_est = aiptw_1[['plug_in_est']],
                         mean_aug = mean(aiptw_1[['augmentation']]), var_aug = aiptw_1[['var_aug']]),
              data.frame(aiptw = aiptw_0[['aiptw']], plug_in_est = aiptw_0[['plug_in_est']],
                         mean_aug = mean(aiptw_0[['augmentation']]), var_aug = aiptw_0[['var_aug']]),
